...
def main():
    ...
    parser.add_argument("-opt", type=str, default="options/train/light.yml")
    parser.add_argument(
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(tqdm(train_loader)):
            ...
            LQ = util.upscale(LQ, (opt["degradation"]["scale"], 1))  #  bicubic, which can be repleced by deep networks
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    LQ = util.upscale(LQ, (opt["degradation"]["scale"], 1), mode='bicubic')
                    lq_img = LQ.detach()
                    ...
                    lq_img = util.tensor2img(lq_img.squeeze())  # uint8

                    # save the validation results
                    save_path = str(opt["path"]["experiments_root"]) + '/val_images/' + str(current_step)
                    util.mkdirs(save_path)
                    save_name = save_path + f'/{idx:03d}_lq_gt_pred.png'
                    save_output = np.concatenate((lq_img, gt_img, output), axis=1)
                    util.save_img(save_output, save_name)
