...
class SRUNet(nn.Module):
    def __init__(self, up_scale=6):
        ...
        self.fracconv1 = nn.ConvTranspose3d(
            in_channels=32,
            out_channels=32,
            kernel_size=3,
            stride=(self.up_scale, 1, 1),
            padding=1,
        )
        ...
        self.fracconv2 = nn.ConvTranspose3d(
            in_channels=64, out_channels=64, kernel_size=3, stride=(2, 1, 1), padding=1
        )
        self.conv3_1 = conv3x3x3(64, 128)
        self.conv3_2 = conv3x3x3(128, 128)
        self.conv3_3 = conv3x3x3(128, 128)
        self.fracconv3 = nn.ConvTranspose3d(
            in_channels=128, out_channels=64, kernel_size=3, stride=(2, 2, 2), padding=1
        )
        self.conv2_4 = conv3x3x3(128, 64)
        self.conv2_5 = conv3x3x3(64, 64)
        self.conv2_6 = conv3x3x3(64, 64)
        self.fracconv4 = nn.ConvTranspose3d(
            in_channels=64,
            out_channels=32,
            kernel_size=3,
            stride=(self.up_scale // 2, 2, 2),
            padding=1,
        )
    ...
    def forward(self, x):
        ...
        x_frac4 = self.fracconv4(
            x_2_7, output_size=(b, c, d * self.up_scale // 2, h * 2, w * 2)
        )
        # print(x_frac4.shape)
